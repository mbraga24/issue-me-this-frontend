// user store
export const SET_LOADING = "SET_LOADING";
export const SET_USERS = "SET_USERS";
export const SET_KEY_HOLDER = "SET_KEY_HOLDER";
export const ADD_USER = "ADD_USER";
export const UPDATE_USER = "UPDATE_USER";

export const REMOVE_KEY_HOLDER_ISSUE_LIKE = "REMOVE_KEY_HOLDER_ISSUE_LIKE";
export const ADD_KEY_HOLDER_ISSUE_LIKE = "ADD_KEY_HOLDER_ISSUE_LIKE";

export const REMOVE_KEY_HOLDER_COMMENT_LIKE = "REMOVE_KEY_HOLDER_COMMENT_LIKE";
export const ADD_KEY_HOLDER_COMMENT_LIKE = "ADD_KEY_HOLDER_COMMENT_LIKE";

export const ADD_KEY_HOLDER_FAVORITE = "ADD_KEY_HOLDER_FAVORITE";
export const REMOVE_KEY_HOLDER_FAVORITE = "REMOVE_KEY_HOLDER_FAVORITE";

// search term store
export const SET_SEARCH_TERM = "SET_SEARCH_TERM";

// issue store 
export const SET_ISSUES = "SET_ISSUES";
export const SET_ISSUE_INDEX = "SET_ISSUE_INDEX";
export const ADD_ISSUE_INDEX = "ADD_ISSUE_INDEX";
export const UPDATE_ISSUE_INDEX = "UPDATE_ISSUE_INDEX";
export const DELETE_ISSUE_INDEX = "DELETE_ISSUE_INDEX";
export const ADD_ISSUE = "ADD_ISSUE";
export const UPDATE_ISSUE = "UPDATE_ISSUE";
export const DELETE_ISSUE = "DELETE_ISSUE";

// comment store
export const SET_COMMENTS = "SET_COMMENTS";
export const ADD_COMMENT = "ADD_COMMENT";
export const UPDATE_COMMENT = "UPDATE_COMMENT";
export const DELETE_COMMENT = "DELETE_COMMENT";

// LikeIssue store
export const SET_ISSUE_LIKES = "SET_ISSUE_LIKES";
export const ADD_ISSUE_LIKE = "ADD_ISSUE_LIKE";
export const DELETE_ISSUE_LIKE = "DELETE_ISSUE_LIKE";

// LikeComment store
export const SET_COMMENT_LIKES = "SET_COMMENT_LIKES";
export const ADD_COMMENT_LIKE = "ADD_COMMENT_LIKE";
export const DELETE_COMMENT_LIKE = "DELETE_COMMENT_LIKE";

// skill store
export const SET_SKILLS = "SET_SKILLS";

// updateForm store
export const UPDATE_TITLE = "UPDATE_TITLE";
export const UPDATE_BODY = "UPDATE_BODY";
export const UPDATE_SYNTAX = "UPDATE_SYNTAX";
export const UPDATE_EMAIL = "UPDATE_EMAIL";
export const UPDATE_LAST_NAME = "UPDATE_LAST_NAME";
export const UPDATE_FIRST_NAME = "UPDATE_FIRST_NAME";
export const UPDATE_JOB_TITLE = "UPDATE_JOB_TITLE";
export const UPDATE_AVATAR = "UPDATE_AVATAR";
export const UPDATE_TOP_SKILLS = "UPDATE_TOP_SKILLS";
export const UPDATE_BIRTHDAY = "UPDATE_BIRTHDAY";